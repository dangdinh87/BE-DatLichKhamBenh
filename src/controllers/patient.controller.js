...
const update = catchAsync(async (req, res) => {
  ...
  if (req.files.image && req.files.image !== null && req.files?.image[0])
    objFiles.image = req.files?.image[0].filename;

  const result = { ...objFiles, ...req.body };
  const updatePatient = await patientService.update(req.params.id, result);
  if (!updatePatient) {
